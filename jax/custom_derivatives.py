...
def linear_call(fun: Callable, fun_transpose: Callable, residual_args,
                linear_args):
  """Call a function, indicating a custom implementation for its transpose.
  ...
  transposes of one another across transposition. Specifically, the
  transpose of a ``linear_call`` primitive is another ``linear_call``
  to ``fun_transpose``, with ``fun`` as its custom transposition.
  ...
  Args:
    fun: a Python callable specifying a structurally linear
      function. It should take two arguments: one of "residual" inputs
      (type ``r``), i.e. inputs in which the function is not
      necessarly linear, and one of "linear" inputs (type ``a``).  It
      should return output whose components are linear in the linear
      input (type ``b``).
  ...
  if t_out_tree() != lin_tree:
    raise TypeError(
        f'transpose output pytree structure must match that of linear inputs, '
...
def _linear_call_transpose_rule(cts, *args, callee, transpose,
  ...
  return ([None] * num_callee_consts +
          [None] * num_transpose_consts +
          [None] * num_res +
          cts_out)
